...
ps_model.fit(df[confounders], df[treatment_col])
df = df.copy()
...
matched_control = control.iloc[indices.flatten()[keep]].reset_index(drop=True)

att = (matched_treated[outcome_col] - matched_control[outcome_col]).mean()
print({"n_matched": len(matched_treated), "att": att})
```

## Balance Helper

```python
def smd(before_or_after, cols, treatment_col):
    treated = before_or_after[before_or_after[treatment_col] == 1]
    control = before_or_after[before_or_after[treatment_col] == 0]
    rows = []
    for col in cols:
        mt, mc = treated[col].mean(), control[col].mean()
        vt, vc = treated[col].var(), control[col].var()
        pooled = np.sqrt((vt + vc) / 2)
        rows.append({"covariate": col, "smd": (mt - mc) / pooled if pooled else np.nan})
